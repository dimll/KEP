...
define('composer/uploads', [
	...
	var uploadingText = '';
	var post_uuid_Custom;

	uploads.initialize = function(post_uuid) {
		post_uuid_Custom = post_uuid;
	...
	function deleteText(str, index, length){
	...
	function uploadContentFilesCustom(params) {
		var files = params.files;
		var trainingSubFolder = params.subFolder;
		var shortDescText = params.description;
		var topicTitle = params.title;
		var post_uuid = post_uuid_Custom;
		var postContainer = $('.composer[data-uuid="' + post_uuid + '"]');
		...
		var titleString = `***\n# ${topicTitle} \n`;
		var stringToAppend = titleString + `#### Description: *${shortDescText}* \n`;

		var regex = /(\*\*\*\n)(#\s)(.*\n)*(\*\*\*\n)/g;
		var foundIndex = text.search(regex) > -1 ? text.search(regex) : 0;
		var foundLength = text.match(regex) && text.match(regex)[0].length ? text.match(regex)[0].length : 0;

		var prependText = text.slice(0,foundIndex);
		var postpendText = text.slice(foundIndex + foundLength);		
		...
		for (var i = 0; i < files.length; ++i) {
			...
			var itemString = '\n' + `File ${i+1}: ` + (isImage ? '!' : '') + '[' + filenameMapping[i] + '](' + uploadingText + ') '
			stringToAppend = insertText(stringToAppend, stringToAppend.length + (itemString.length * i), itemString);
		}
		stringToAppend = insertText(stringToAppend, stringToAppend.length, "\n***\n");
		if (uploadForm.length) {
			postContainer.find('[data-action="post"]').prop('disabled', true);
		}
		textarea.value = `${prependText}${stringToAppend}${postpendText}`;
		...
		uploadForm.off('submit').submit(function() {
			function updateTextArea(filename, text, trim) {
				...
				var re = new RegExp(escapeRegExp(filename) + "]\\([^)]+\\)", 'g');
				textarea.value = current.replace(re, (newFilename || filename) + '](' + text + ')');
	...
	uploads.uploadContentFiles = function(params) {
		uploadContentFilesCustom(params);
	}
