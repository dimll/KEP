    ...
    async.waterfall([
        ...
        (packs, customizations, next) => {
            ...
            customizations.adjuncts.forEach((adjunct) => {
                ...
                table[name] = Object.assign(Object.assign({}, table[name]), { aliases: table[name].aliases.concat(adjunct.aliases) });
            ...
            async.parallel([
              ...
              margin-bottom: -1px;
                ...
                (cb) => async.each(packs, (pack, next2) => {
                    ...
                    if (pack.mode === 'images') {
                        linkDirs(path_1.resolve(pack.path, pack.images.directory), dir, next2);
                    }
                    else if (pack.mode === 'sprite') {
                        const filename = path_1.basename(pack.sprite.file);
                        async.series([
                            (cb2) => fs_extra_1.mkdirp(dir, cb2),
                            (cb2) => fs_extra_1.copy(path_1.resolve(pack.path, pack.sprite.file), path_1.join(dir, filename), cb2),
                        ], next2);
                    }
                    else { // pack.mode === 'font'
                        const fontFiles = [
                        ...
                        async.series([
                            ...
                            (cb2) => async.each(fontFiles, (file, next3) => {
                                ...
                                fs_extra_1.copy(path_1.resolve(pack.path, file), path_1.join(dir, filename), next3);
                            }, cb2),
                        ], next2);
                    }
                }, cb),
